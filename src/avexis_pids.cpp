...
void AvexisPids::UpdatePositionPID()
{
...
void AvexisPids::UpdateVelocityPID()
{
    for(std::vector<pid_st>::iterator pid = velocity_pids_.begin(); pid!=velocity_pids_.end();++pid)
        *(pid->command_ptr) = pid->pid.computeCommand(*(pid->error_ptr), dt_);
}

bool AvexisPids::ToPositionControl(std_srvs::EmptyRequest &_req, std_srvs::EmptyResponse &_res)
{
...
bool AvexisPids::ToVelocityControl(std_srvs::EmptyRequest &_req, std_srvs::EmptyResponse &_res)
{
...
void AvexisPids::InitPID(control_toolbox::Pid &_pid, const ros::NodeHandle&_node, const bool &_use_dynamic_reconfig)
{
