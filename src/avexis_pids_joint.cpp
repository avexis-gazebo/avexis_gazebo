...
void AvexisJointPids::Init(const ros::NodeHandle&_node, ros::Duration&_dt)
{
    // init dt from rate
    dt_ = _dt;
    pid_node_ = _node;
...
bool AvexisJointPids::UpdatePID()
{
    if(!vmax_is_set_)
    {
        double dummy;
...
void AvexisJointPids::SetpointCallBack(const sensor_msgs::JointStateConstPtr &_msg)
{
...
void AvexisJointPids::MeasureCallBack(const sensor_msgs::JointStateConstPtr &_msg)
{
