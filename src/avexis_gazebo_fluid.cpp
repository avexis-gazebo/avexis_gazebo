...
void AvexisFluidPlugin::ReadVector3(const std::string &_string, math::Vector3 &_vector)
{
...
void AvexisFluidPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
    ROS_INFO("Loading avexis_fluid plugin");
    this->world_ = _world;
    ...
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<geometry_msgs::Vector3>(
                fluid_topic, 1,
                boost::bind(&AvexisFluidPlugin::FluidVelocityCallBack, this, _1),
                ros::VoidPtr(), &callback_queue_);
    fluid_velocity_.Set(0,0,0);
    fluid_velocity_subscriber_ = rosnode_->subscribe(ops);

    // Register plugin update
    update_event_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&AvexisFluidPlugin::Update, this));

    // Clear existing links
    buoyant_links_.clear();
    parsed_models_.clear();

    ROS_INFO("Loaded avexis_fluid plugin.");
}

void AvexisFluidPlugin::Update()
{
    // activate callbacks
    ...
    for( std::vector<link_st>::iterator link_it = buoyant_links_.begin(); link_it!=buoyant_links_.end();++link_it)
    {
        ...
        for(model_it = parsed_models_.begin(); model_it!=parsed_models_.end();++model_it)
        {
            ...
            // Add a motherfucking timestamp so this data is actually usable and plottable.
...
void AvexisFluidPlugin::ParseNewModel(const physics::ModelPtr &_model)
{
...
void AvexisFluidPlugin::RemoveDeletedModel(std::vector<model_st>::iterator &_model_it)
{
...
void AvexisFluidPlugin::FluidVelocityCallBack(const geometry_msgs::Vector3ConstPtr &_msg)
{
