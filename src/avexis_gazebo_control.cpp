...
void AvexisControlPlugin::ReadVector3(const std::string &_string, math::Vector3 &_vector)
{
...
bool AvexisControlPlugin::SwitchService(std_srvs::EmptyRequest &req, std_srvs::EmptyResponse &res)
{
    if(controller_is_running_)
        ROS_INFO("Switching avexis_control OFF");
    else
        ROS_INFO("Switching avexis_control ON");
    controller_is_running_ = !controller_is_running_;
    return true;
}

void AvexisControlPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    // get model and name
    model_ = _model;
    ...
    if(control_body_)
    {
        ...
        ros::SubscribeOptions ops = ros::SubscribeOptions::create<geometry_msgs::Wrench>(
                    ...
                    boost::bind(&AvexisControlPlugin::BodyCommandCallBack, this, _1),
    ...
    if(control_joints_ && model_->GetJointCount() != 0)
    {
        ...
        ros::SubscribeOptions ops = ros::SubscribeOptions::create<sensor_msgs::JointState>(
                    ...
                    boost::bind(&AvexisControlPlugin::JointCommandCallBack, this, _1),
                    ros::VoidPtr(), &callback_queue_);
    ...
    //switch_service_ = rosnode_.advertiseService<std_srvs::EmptyRequest, std_srvs::EmptyResponse>("switch", &AvexisControlPlugin::SwitchService, this);
    ...
    update_event_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&AvexisControlPlugin::Update, this));

    ros::spinOnce();
    ROS_INFO("Started Avexis Control Plugin for %s.", _model->GetName().c_str());
}

void AvexisControlPlugin::Update()
{
    // activate callbacks
...
void AvexisControlPlugin::ComputePseudoInverse(const Eigen::MatrixXd &_M, Eigen::MatrixXd &_pinv_M)
{
...
void AvexisControlPlugin::BodyCommandCallBack(const geometry_msgs::WrenchConstPtr &_msg)
{
    if(!control_body_)
        return;
